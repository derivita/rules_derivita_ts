...
export function gatherDiagnostics(
    ...
    ignoreForDiagnostics: Set<ts.SourceFile> = new Set(),
    host:ts.ModuleResolutionHost): ts.Diagnostic[] {
  ...
  perfTrace.wrap('type checking', () => {
    ...
    plugins.push(
        ...getCommonPlugins(options, bazelOpts, program, disabledTsetseRules, host));
...
export function getCommonPlugins(
    ...
    host:ts.ModuleResolutionHost): DiagnosticPlugin[] {
  ...
  if (!bazelOpts.isJsTranspilation) {
    let tsetsePluginConstructor:
        {new (program: ts.Program, host:ts.ModuleResolutionHost, disabledRules: string[]): DiagnosticPlugin} =
            BazelConformancePlugin;
    plugins.push(new tsetsePluginConstructor(program, host, disabledTsetseRules));
  }
  return plugins;
}
...
export async function createProgramAndEmit(
  ...
  if (bazelOpts.angularCompilerOptions) {
      return {
        diagnostics: [errorDiag(
            '`ts_library(use_angular_plugin=True)` is not supported ')]
      };
  }
  ...
  if (!bazelOpts.isJsTranspilation) {
    ...
    let diagnostics = gatherDiagnostics(
        ...
        ignoreForDiagnostics, compilerHost);
...
export function emitWithTsickle(
  ...
  try {
    // tslint:disable-next-line:no-require-imports
    optTsickle = require('tsickle/tsickle');
  } catch (e:any) {
