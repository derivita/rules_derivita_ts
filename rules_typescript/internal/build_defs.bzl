...
load("//third_party/rules_typescript/internal:common/compilation.bzl", "COMMON_ATTRIBUTES", "DEPS_ASPECTS", "compile_ts", "ts_providers_dict_to_struct")
load("//third_party/rules_typescript/internal:common/tsconfig.bzl", "create_tsconfig")
load("//third_party/rules_typescript/internal:ts_config.bzl", "TsConfigInfo")
...
_DEFAULT_COMPILER = "//third_party/rules_typescript/internal:tsc_wrapped_bin"
...
_DEVMODE_TARGET_DEFAULT = "es2017"
_DEVMODE_MODULE_DEFAULT = "umd"
_PRODMODE_TARGET_DEFAULT = "es2017"
_PRODMODE_MODULE_DEFAULT = "esnext"

TS_ATTRIBUTES = dict(COMMON_ATTRIBUTES, **{
...
- Give an explicit `tsconfig` attribute to all `ts_library` targets
    ...
    "tsickle": attr.bool(default = True, doc = "Whether to enable tsickle."),
...
def compile_action(ctx, inputs, outputs, tsconfig_file, node_opts, description = "prodmode"):
    externs_files = []
    action_inputs = inputs
    action_outputs = []
    for output in outputs:
        if output.basename.endswith(".externs.js") and not ctx.attr.tsickle:
...
def devmode_compile_action(ctx, inputs, outputs, tsconfig_file, node_opts):
    compile_action(
        ctx,
        inputs,
        outputs,
...
def _ts_library_impl(ctx):
    ...
    ts_providers = compile_ts(
        ...
        devmode_compile_action = devmode_compile_action,
