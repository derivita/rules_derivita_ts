...
function checkCallExpression(checker: Checker, node: ts.CallExpression) {
  ...
  if (tsutils.isThenableType(checker.typeChecker, node)) {
    checker.addFailureAtNode(node, FAILURE_STRING, undefined, undefined);
  }
}
  ...
  for (let inode = node.parent; inode !== undefined; inode = inode.parent) {
    switch (inode.kind) {
      ...
      case ts.SyntaxKind.MethodDeclaration:
        // Potentially async
        // Hacky?
        const modifiers = ts.canHaveModifiers(inode) ? ts.getModifiers(inode) : undefined;
        return tsutils.hasModifier(modifiers as any, ts.SyntaxKind.AsyncKeyword);
