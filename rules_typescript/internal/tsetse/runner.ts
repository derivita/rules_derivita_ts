...
import { DERIVITA_TSEC_RULES } from './tsec/derivita_config';
...
export class Plugin implements pluginApi.DiagnosticPlugin {
  ...
  constructor(program: ts.Program, host: ts.ModuleResolutionHost, disabledTsetseRules: string[] = []) {
    this.checker = new Checker(program, host);
