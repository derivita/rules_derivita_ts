...
load("//tools/workspace_name:workspace_name_helper.bzl", "get_workspace_name")
...
def _to_manifest_path(ctx, file):
    if file.short_path.startswith("../"):
        ...
        # We use the regular workspace name here because otherwise the path to the devserver_bin file
        # ends up being wrong, preventing storybook from working.
...
def _concatjs_devserver(ctx):
    ...
    workspace_name = ctx.label.workspace_name if ctx.label.workspace_name else get_workspace_name(ctx)
...
concatjs_devserver = rule(
    ...
    attrs = {
        ...
        "devserver": attr.label(
            ...
            default = Label("//third_party/rules_typescript/devserver:devserver_bin"),
            executable = True,
            cfg = "exec",
        ),
        ...
        "_launcher_template": attr.label(allow_single_file = True, default = Label("//third_party/rules_typescript/devserver:launcher_template.sh")),
        "_requirejs_script": attr.label(allow_single_file = True, default = Label("//third_party/rules_typescript/third_party/npm/requirejs:require.js")),
    },
    outputs = {
        "manifest": "%{name}.MF",
        "script": "%{name}.sh",
