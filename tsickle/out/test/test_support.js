...
const cliSupport = __importStar(require("../src/cli_support"));
...
class GoldenFileTest {
    ...
    get isNamespaceTransformationEnabled() {
...
function goldenTests() {
    ...
    if (process.env['TESTBRIDGE_TEST_ONLY']) {
        ...
        if (tests.length === 0) {
            console.error(`'--test_filter=${process.env['TESTBRIDGE_TEST_ONLY']}' did not match any tests`);
            process.exit(1);
        }
    }
    return tests;
 ...
 */
function pathToModuleName(rootModulePath, context, fileName, options, host) {
