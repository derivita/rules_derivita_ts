    ...
    return (context) => {
        return (src) => {
            ...
            if (!generator.foundMigrationExportsShim()) {
                ...
                const fileSummary = new summary_1.FileSummary();
                fileSummary.moduleType = summary_1.ModuleType.UNKNOWN;
...
class Generator {
    ...
    generateExportShimJavaScript() {
        ...
        const content = lines('/**', ' * @fileoverview generator:ts_migration_exports_shim.ts', ' * original_file:' + this.srcIds.google3Path, ` * ${pintoModuleAnnotation}`, ' */', `goog.module('${this.outputIds.googModuleId}');`, maybeDeclareLegacyNameCall, mainModuleRequire, exportsAssignment, '');
        const fileSummary = new summary_1.FileSummary();
        fileSummary.addProvide({ type: summary_1.Type.CLOSURE, name: this.outputIds.googModuleId });
        fileSummary.addStrongRequire({ type: summary_1.Type.CLOSURE, name: 'goog' });
        fileSummary.addStrongRequire({ type: summary_1.Type.CLOSURE, name: this.srcIds.googModuleId });
        fileSummary.autochunk = isAutoChunk;
        fileSummary.moduleType = summary_1.ModuleType.GOOG_MODULE;
