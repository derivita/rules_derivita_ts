...
function isInUnsupportedNamespace(node) {
    ...
    while (parent) {
        if (parent.kind === ts.SyntaxKind.ModuleDeclaration) {
            return !(0, transformer_util_1.isMergedDeclaration)(parent);
        }
            ...
            for (const member of node.members) {
                ...
                if (member.initializer) {
                    ...
                    if (typeof enumConstValue === 'number') {
                        ...
                        if (enumConstValue < 0) {
                            enumValue = ts.factory.createPrefixUnaryExpression(ts.SyntaxKind.MinusToken, ts.factory.createNumericLiteral(-enumConstValue));
                        }
                        else {
