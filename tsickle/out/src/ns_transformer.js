    ...
    return (context) => {
        return (sourceFile) => {
            ...
            function transformNamespace(ns, mergedDecl) {
                ...
                for (const stmt of ns.body.statements) {
                    ...
                    if (ts.isClassDeclaration(stmt)) {
                        transformInnerDeclaration(stmt, (classDecl, notExported, hoistedIdent) => {
                            return ts.factory.updateClassDeclaration(classDecl, notExported, hoistedIdent, classDecl.typeParameters, classDecl.heritageClauses, classDecl.members);
                        });
                    }
                    else if (ts.isEnumDeclaration(stmt)) {
                        transformInnerDeclaration(stmt, (enumDecl, notExported, hoistedIdent) => {
                            return ts.factory.updateEnumDeclaration(enumDecl, notExported, hoistedIdent, enumDecl.members);
                        });
                    }
                    else if (ts.isInterfaceDeclaration(stmt)) {
                        transformInnerDeclaration(stmt, (interfDecl, notExported, hoistedIdent) => {
                            return ts.factory.updateInterfaceDeclaration(interfDecl, notExported, hoistedIdent, interfDecl.typeParameters, interfDecl.heritageClauses, interfDecl.members);
                        });
                    }
                ...
                (0, transformer_util_1.markAsMergedDeclaration)(ns);
                (0, transformer_util_1.markAsMergedDeclaration)(mergedDecl);
                ...
                function transformConstDeclaration(varDecl) {
                    for (let decl of varDecl.declarationList.declarations) {
                        ...
                        const originalName = (0, transformer_util_1.getIdentifierText)(decl.name);
                ...
                function transformTypeAliasDeclaration(aliasDecl) {
                    ...
                    const originalName = (0, transformer_util_1.getIdentifierText)(aliasDecl.name);
                    if (!(0, transformer_util_1.hasModifierFlag)(aliasDecl, ts.ModifierFlags.Export)) {
                        error(aliasDecl, `'${originalName}' must be exported. (go/ts-merged-namespaces)`);
                    }
                    aliasDecl = fixReferences(aliasDecl);
                    const notExported = ts.factory.createModifiersFromModifierFlags(ts.getCombinedModifierFlags(aliasDecl) &
                        (~ts.ModifierFlags.Export));
                    aliasDecl = ts.factory.updateTypeAliasDeclaration(aliasDecl, notExported, aliasDecl.name, aliasDecl.typeParameters, aliasDecl.type);
                ...
                function transformInnerDeclaration(decl, updateDecl) {
                    ...
                    const notExported = ts.factory.createModifiersFromModifierFlags(ts.getCombinedModifierFlags(decl) & (~ts.ModifierFlags.Export));
                ...
                function createInnerNameAlias(propName, initializer, original) {
                    const prop = ts.factory.createExpressionStatement(ts.factory.createAssignment(ts.factory.createPropertyAccessExpression(mergedDecl.name, propName), initializer));
                ...
                function maybeFixIdentifier(ident) {
                    if (isNamespaceRef(ident)) {
                        ...
                        const nsProp = ts.factory.createPropertyAccessExpression(nsIdentifier, ident);
                ...
                function maybeFixPropertyAccess(prop) {
                    if (ts.isPropertyAccessExpression(prop.expression)) {
                        ...
                        if (updatedProp !== prop.expression) {
                            return ts.factory.updatePropertyAccessExpression(prop, updatedProp, prop.name);
                        }
                        return prop;
                    }
                    ...
                    if (nsProp !== prop.expression) {
                        const newPropAccess = ts.factory.updatePropertyAccessExpression(prop, nsProp, prop.name);
            ...
            function visitTopLevelStatement(node) {
                ...
                const mergedDecl = (0, transformer_util_1.getPreviousDeclaration)(sym, ns);
