...
exports.decoratorDownlevelTransformer = void 0;
/**
...
function createDecoratorClassProperty(decoratorList) {
    ...
    const prop = ts.factory.createPropertyDeclaration([modifier], 'decorators', undefined, undefined, initializer);
    ...
    return (context) => {
        ...
        function transformClassElement(element) {
            ...
            const modifiers = modifiersToKeep.length ?
                ts.setTextRange(ts.factory.createNodeArray(modifiersToKeep), ts.factory.createNodeArray((_a = element.modifiers) !== null && _a !== void 0 ? _a : [])) :
                undefined;
            switch (element.kind) {
                case ts.SyntaxKind.PropertyDeclaration:
                    newNode = ts.factory.updatePropertyDeclaration(element, modifiers, element.name, (_b = element.questionToken) !== null && _b !== void 0 ? _b : element.exclamationToken, element.type, element.initializer);
                    break;
                case ts.SyntaxKind.GetAccessor:
                    newNode = ts.factory.updateGetAccessorDeclaration(element, modifiers, element.name, element.parameters, element.type, element.body);
                    break;
                case ts.SyntaxKind.SetAccessor:
                    newNode = ts.factory.updateSetAccessorDeclaration(element, modifiers, element.name, element.parameters, element.body);
                    break;
                case ts.SyntaxKind.MethodDeclaration:
                    newNode = ts.factory.updateMethodDeclaration(element, modifiers, element.asteriskToken, element.name, element.questionToken, element.typeParameters, element.parameters, element.type, element.body);
                    break;
                default:
        ...
        function transformConstructor(ctor) {
            ...
            for (const param of oldParameters) {
                ...
                const newParam = ts.factory.updateParameterDeclaration(param, // Must pass 'undefined' to avoid emitting decorator
                // metadata.
                modifiersToKeep, param.dotDotDotToken, param.name, param.questionToken, param.type, param.initializer);
                newParameters.push(newParam);
            }
            const updated = ts.factory.updateConstructorDeclaration(ctor, ctor.modifiers, newParameters, ts.visitFunctionBody(ctor.body, visitor, context));
        ...
        function transformClassDeclaration(classDecl) {
            ...
            return ts.factory.updateClassDeclaration(classDecl, modifiersToKeep.length ? modifiersToKeep : undefined, classDecl.name, classDecl.typeParameters, classDecl.heritageClauses, ts.setTextRange(ts.factory.createNodeArray(newMembers, classDecl.members.hasTrailingComma), classDecl.members));
        }
