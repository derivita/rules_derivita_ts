...
exports.ModuleTypeTranslator = void 0;
/**
...
const typeTranslator = __importStar(require("./type_translator"));
...
class ModuleTypeTranslator {
    constructor(sourceFile, typeChecker, host, diagnostics, isForExterns, useInternalNamespaceForExterns = false) {
        ...
        this.host.unknownTypesPaths =
            ...
            (_a = this.host.unknownTypesPaths) !== null && _a !== void 0 ? _a : this.host.typeBlackListPaths;
    }
    ...
    addRequireTypeIfIsExported(decl, sym) {
        ...
        if (!(0, transformer_util_1.hasModifierFlag)(decl, ts.ModifierFlags.ExportDefault))
            return false;
     ...
     *     emit a `.default`.
     */
    requireType(context, importPath, moduleSymbol, isDefaultImport = false, leaveModulesAlone = false) {
        if (this.host.untyped)
            return;
        ...
        const nsImport = googmodule.jsPathToNamespace(this.host, context, this.diagnostics, importPath, () => moduleSymbol);
        ...
        if (googmodule.jsPathToStripProperty(this.host, importPath, () => moduleSymbol)) {
        ...
        this.additionalImports.push(leaveModulesAlone ?
            ts.factory.createImportDeclaration(undefined, ts.factory.createImportClause(false, undefined, ts.factory.createNamespaceImport(ts.factory.createIdentifier(requireTypePrefix))), ts.factory.createStringLiteral(importPath))
            : ts.factory.createVariableStatement(undefined, ts.factory.createVariableDeclarationList([ts.factory.createVariableDeclaration(requireTypePrefix, /* exclamationToken */ undefined, 
    ...
    registerExternSymbolAliases(importPath, moduleSymbol) {
        const moduleNamespace = (0, annotator_host_1.moduleNameAsIdentifier)(this.host, importPath, this.sourceFile.fileName);
        for (let sym of this.typeChecker.getExportsOfModule(moduleSymbol)) {
            // Some users import {default as SomeAlias} from 'goog:...';
            // The code below must recognize this as a default import to alias the
            // symbol to just the blank module name.
    ...
    ensureSymbolDeclared(sym) {
        ...
        const clutzModule = this.typeChecker.getSymbolsInScope(clutzDts, ts.SymbolFlags.Module)
            .find((module) => {
            var _a;
            return module.getName().startsWith('"goog:') &&
                ((_a = module.valueDeclaration) === null || _a === void 0 ? void 0 : _a.getSourceFile()) === clutzDts &&
                this.typeChecker.getExportsOfModule(module).some((exported) => {
    ...
    getJSDoc(node, reportWarnings) {
        return jsdoc.getJSDocTags(node, reportWarnings ? this.diagnostics : undefined, this.sourceFile);
    }
    ...
    getFunctionTypeJSDoc(fnDecls, extraTags = []) {
        ...
        for (const fnDecl of fnDecls) {
            ...
            argCounts.push(hasThisParam ? sig.declaration.parameters.length - 1 :
