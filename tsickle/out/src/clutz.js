...
function ambientModuleSymbolFromClutz(googmoduleHost, typeChecker, stmt) {
    ...
    const namespace = googmodule.jsPathToNamespace(googmoduleHost, stmt, ignoredDiagnostics, stmt.moduleSpecifier.text, () => moduleSymbol);
    if (namespace === null)
        return undefined;
    return moduleSymbol;
}
...
function clutzSymbolFromQualifiedName(typeChecker, name) {
    ...
    if (!sym || !sym.declarations || sym.declarations.length === 0 ||
        !(0, type_translator_1.isDeclaredInClutzDts)(sym.declarations[0])) {
        return undefined;
    }
    return sym;
}
