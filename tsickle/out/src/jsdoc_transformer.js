...
const transformerUtil = __importStar(require("./transformer_util"));
    ...
    return (context) => {
        return (sourceFile) => {
            ...
            function visitInterfaceDeclaration(iface) {
                ...
                if ((0, transformer_util_1.symbolIsValue)(typeChecker, sym) && !(0, transformer_util_1.isMergedDeclaration)(iface)) {
                ...
                const modifiers = transformerUtil.hasModifierFlag(iface, ts.ModifierFlags.Export) ?
                    ...
                    undefined;
                const decl = ts.setSourceMapRange(ts.factory.createFunctionDeclaration(modifiers, 
                ...
                const isFirstOccurrence = (0, transformer_util_1.getPreviousDeclaration)(sym, iface) === null;
                const declarations = [];
            ...
            function visitFunctionLikeDeclaration(fnDecl) {
                ...
                for (const param of fnDecl.parameters) {
                    ...
                    updatedParams.push(ts.factory.updateParameterDeclaration(param, param.modifiers, param.dotDotDotToken, updatedParamName, param.questionToken, param.type, param.initializer));
                }
                ...
                switch (fnDecl.kind) {
                    case ts.SyntaxKind.FunctionDeclaration:
                        fnDecl = ts.factory.updateFunctionDeclaration(fnDecl, fnDecl.modifiers, fnDecl.asteriskToken, fnDecl.name, fnDecl.typeParameters, updatedParams, fnDecl.type, body);
                        break;
                    case ts.SyntaxKind.MethodDeclaration:
                        fnDecl =
                            ts.factory.updateMethodDeclaration(fnDecl, fnDecl.modifiers, fnDecl.asteriskToken, fnDecl.name, fnDecl.questionToken, fnDecl.typeParameters, updatedParams, fnDecl.type, body);
                        break;
                    case ts.SyntaxKind.SetAccessor:
                        fnDecl = ts.factory.updateSetAccessorDeclaration(fnDecl, fnDecl.modifiers, fnDecl.name, updatedParams, body);
                        break;
                    case ts.SyntaxKind.Constructor:
                        fnDecl = ts.factory.updateConstructorDeclaration(fnDecl, fnDecl.modifiers, updatedParams, body);
                        break;
                ...
                for (const decl of varStmt.declarationList.declarations) {
                    ...
                    if (ts.isIdentifier(decl.name)) {
                        ...
                        if (!initializersMarkedAsUnknown &&
                            ...
                            ((_a = decl.initializer) === null || _a === void 0 ? void 0 : _a.kind) !== ts.SyntaxKind.ClassExpression) {
            ...
            function visitTypeAliasDeclaration(typeAlias) {
                ...
                if (ns !== null &&
                    (((_a = ts.getOriginalNode(typeAlias).parent) === null || _a === void 0 ? void 0 : _a.parent) === ns) &&
                ...
                if (propertyBase !== null) {
                    decl = ts.factory.createExpressionStatement(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier(propertyBase), ts.factory.createIdentifier(typeName)));
                }
                else {
                    // Given: type T = ...;
                    // We produce: var T;
            ...
            function getNarrowedType(node) {
                ...
                if ((symbol === null || symbol === void 0 ? void 0 : symbol.declarations) === undefined ||
            ...
            function visitPropertyAccessExpression(node) {
                ...
                const propertyAccessWithCast = ts.factory.updatePropertyAccessExpression(node, createClosureCast(node.expression, ts.visitEachChild(node.expression, visitor, context), objType), node.name);
            ...
            function visitImportDeclaration(importDecl, leaveModulesAlone = false) {
             ...
             */
            function visitExportDeclaration(exportDecl, leaveModulesAlone = false) {
                ...
                if (importedModuleSymbol) {
                    ...
                    /* default import? */ false, leaveModulesAlone);
                }
                const typesToExport = [];
                if (!exportDecl.exportClause) {
                    ...
                    exportDecl = ts.factory.updateExportDeclaration(exportDecl, exportDecl.modifiers, isTypeOnlyExport, ts.factory.createNamedExports(exportSpecifiers), exportDecl.moduleSpecifier, exportDecl.assertClause);
                }
                ...
                for (const [exportedName, sym] of typesToExport) {
                    ...
                    if (!isTypeAlias || leaveModulesAlone)
                        continue;
            ...
            function renameArrayBindings(node, aliases) {
                ...
                for (const e of node.elements) {
                    ...
                    updatedElements.push(ts.factory.updateBindingElement(e, e.dotDotDotToken, ts.visitNode(e.propertyName, visitor, ts.isPropertyName), updatedBindingName, 
                    // TODO: go/ts50upgrade - Remove after upgrade.
                    // tslint:disable-next-line:no-unnecessary-type-assertion
                    ts.visitNode(e.initializer, visitor)));
                }
            ...
            function visitForOfStatement(node) {
                ...
                return ts.factory.updateForOfStatement(node, node.awaitModifier, updatedInitializer, 
                // TODO: go/ts50upgrade - Remove after upgrade.
                // tslint:disable-next-line:no-unnecessary-type-assertion
            ...
            function visitor(node) {
                ...
                switch (node.kind) {
                    case ts.SyntaxKind.ImportDeclaration:
                        return visitImportDeclaration(node, leaveModulesAlone);
                    case ts.SyntaxKind.ExportDeclaration:
                        return visitExportDeclaration(node, leaveModulesAlone);
                    case ts.SyntaxKind.ClassDeclaration:
